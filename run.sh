#!/usr/bin/env bash
# Eval node: fan gen_eval.py workers across GPUs, then aggregate + upload.
set -eo pipefail
...
python inference/download_weights.py --out /shared/mva-loras >/dev/null 2>&1 || \
    python inference/download_weights.py --out /shared/mva-loras
python scripts/gen_eval.py --plan

NGPU=$(python -c "import json;print(json.load(open('eval_config.json'))['gpus'])")
echo "fanning $NGPU workers"
pids=()
for i in $(seq 0 $((NGPU - 1))); do
  ( sleep $((i * 45)); CUDA_VISIBLE_DEVICES=$i python scripts/gen_eval.py \
      --worker "$i" --workers "$NGPU" 2>&1 | sed "s/^/[w$i] /" ) &
  pids+=($!)
done
rc=0
